test_that("snp_slice basic functionality works", {
  ...
  # Test basic run
  result <- snp_slice(test_data, n_sample = 25, verbose = FALSE)
...
test_that("snp_slice validates parameters", {
  ...
  # Test invalid n_sample
  expect_error(snp_slice(test_data, n_sample = 0), "n_sample must be a positive integer")
  expect_error(snp_slice(test_data, n_sample = 1.5), "n_sample must be a positive integer")
})
...
test_that("snp_slice works with different models", {
  ...
  result_nb <- snp_slice(test_data, model = "negative_binomial", n_sample = 25, verbose = FALSE)
  expect_equal(result_nb$model_info$model, "negative_binomial")
  
  # Test Poisson model
  result_pois <- snp_slice(test_data, model = "poisson", n_sample = 25, verbose = FALSE)
  expect_equal(result_pois$model_info$model, "poisson")
  
  # Test binomial model
  result_bin <- snp_slice(test_data, model = "binomial", n_sample = 25, verbose = FALSE)
...
test_that("snp_slice works with categorical data", {
  ...
  result <- snp_slice(cat_data, model = "categorical", n_sample = 25, verbose = FALSE)
...
test_that("snp_slice model-specific constructors work", {
  ...
  result_cat <- snp_slice_categorical(cat_data, n_sample = 25, verbose = FALSE)
  expect_equal(result_cat$model_info$model, "categorical")
  
  # Test Poisson constructor
  result_pois <- snp_slice_poisson(test_data, n_sample = 25, verbose = FALSE)
  expect_equal(result_pois$model_info$model, "poisson")
  
  # Test binomial constructor
  result_bin <- snp_slice_binomial(test_data, n_sample = 25, verbose = FALSE)
  expect_equal(result_bin$model_info$model, "binomial")
  
  # Test negative binomial constructor
  result_nb <- snp_slice_negative_binomial(test_data, n_sample = 25, verbose = FALSE)
  expect_equal(result_nb$model_info$model, "negative_binomial")
})
...
test_that("snp_slice handles MCMC parameters correctly", {
  ...
  # Test with custom burn-in
  result <- snp_slice(test_data, n_sample = 25, n_burnin = 5, verbose = FALSE)
  expect_equal(result$parameters$n_burnin, 5)
  
  # Test with gap parameter
  result <- snp_slice(test_data, n_sample = 25, gap = 5, verbose = FALSE)
...
test_that("snp_slice stores MCMC samples when requested", {
  ...
  result_no_samples <- snp_slice(test_data, n_sample = 25, store_mcmc = FALSE, verbose = FALSE)
  expect_null(get_chain(result_no_samples)$mcmc_samples)
  
  # Test with storing samples
  result_with_samples <- snp_slice(test_data, n_sample = 25, store_mcmc = TRUE, verbose = FALSE)
  expect_true(is.list(get_chain(result_with_samples)$mcmc_samples))
  expect_equal(length(get_chain(result_with_samples)$mcmc_samples), 25)
})
