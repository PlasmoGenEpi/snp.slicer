# Authors and Citation

## Authors

- **Alfred Hubbard**. Author. [](https://orcid.org/0000-0003-4917-7272)

## Citation

Source:
[`inst/CITATION`](https://github.com/plasmogenepi/snp.slicer/blob/main/inst/CITATION)

Ju, Nianqiao, Liu, Jiawei, He, Qixin (2024). “SNP-slice Resolves Mixed
Infections: Simultaneously Unveiling Strain Haplotypes and Linking Them
to Hosts.” *Bioinformatics*, **40**(6), btae344.
[doi:10.1093/bioinformatics/btae344](https://doi.org/10.1093/bioinformatics/btae344).

    @Article{,
      title = {{{SNP-slice}} Resolves Mixed Infections: Simultaneously Unveiling Strain Haplotypes and Linking Them to Hosts},
      author = {{Ju} and {Nianqiao} and {Liu} and {Jiawei} and {He} and {Qixin}},
      journal = {Bioinformatics},
      year = {2024},
      volume = {40},
      number = {6},
      pages = {btae344},
      doi = {10.1093/bioinformatics/btae344},
    }
