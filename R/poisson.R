...
#'
#' @description
#' Implementation of the Poisson observation model for SNP-Slice.
#' This model assumes Y ~ Poisson(R × proportions).

#' Log-likelihood for Poisson model (matrix version)
#'
...
poisson_loglikelihood_matrix <- function(A, D, model_obj) {
  start_timer("poisson_loglikelihood_matrix")
  
  # Calculate proportions
  proportions <- (A %*% D) / rowSums(A)
  
  # Poisson likelihood: Y ~ Poisson(R × proportions)
  loglik <- sum(stats::dpois(model_obj$y, model_obj$r * proportions, log = TRUE), na.rm = TRUE)
  
  end_timer("poisson_loglikelihood_matrix")
  return(loglik)
}

#' Log-likelihood for Poisson model (vector version)
#'
...
#' @keywords internal
#' @importFrom stats dpois
poisson_loglikelihood_vector <- function(propvec, yvec, rvec) {
  # Poisson likelihood for vectors
  loglik <- sum(stats::dpois(yvec, rvec * propvec, log = TRUE), na.rm = TRUE)
  return(loglik)
}

#' Initialize state for Poisson model
#'
...
poisson_initialize_state <- function(model_obj, threshold = 0.001) {
  y <- model_obj$y
  r <- model_obj$r
  N <- model_obj$N
  P <- model_obj$P
  ...
  if (nsingleppl >= 1) {
    ...
    # Assign mixed infections randomly to subset of strains
    for (imixed in which_mixed) {
      num_to_sample <- min(2, max(1, as.integer(0.25 * nstrain)))
      state$A[imixed, sample.int(nstrain, num_to_sample)] <- 1
    }
    ...
    # Resolve exceptions
    state$loglik <- poisson_loglikelihood_matrix(state$A, state$D, model_obj)
    iter <- 1
    while (is.infinite(state$loglik)) {
      state <- poisson_resolve_exceptions(state, model_obj)
      iter <- iter + 1
      if (iter > 10) {
...
#' Resolve exceptions for Poisson model
#'
...
#' @keywords internal
poisson_resolve_exceptions <- function(state, model_obj) {
  y <- model_obj$y
